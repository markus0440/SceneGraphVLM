...
export COMET_EXPERIMENT_NAME="${COMET_EXPERIMENT_NAME:-Qwen3.5-0.8B | PVSG base_annot train clean | psfr val clean | epochs=${EPOCHS}}"

NUM_EXTRA=()
[[ -z "${CUDA_VISIBLE_DEVICES:-}" ]] && NUM_EXTRA+=(--num_gpus "${NUM_GPUS:-4}")

exec "$SCRIPT_DIR/run_sft.sh" \
  --train "$PVSG/pvsg_base_annot_gt_prompt/train_clean.jsonl" \
  --test "$PVSG/pvsg_psfr_gt_prompt/test_clean.jsonl" \
