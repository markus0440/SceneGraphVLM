#!/usr/bin/env bash
# 3RScan psfr (with_prev_gt): train and validate on PSFR subset.
#
# Run (from this directory):
#   CUDA_VISIBLE_DEVICES=0 ./run_3rscan_psfr.sh
#   CUDA_VISIBLE_DEVICES=0,1,2,3 ./run_3rscan_psfr.sh
#   EPOCHS=10 ./run_3rscan_psfr.sh
...
SUBSET="$R3SCAN/3RScan_json_psfr_with_prev_gt"

EPOCHS="${EPOCHS:-5}"
export COMET_PROJECT_NAME="${COMET_PROJECT_NAME:-qwen_3_5_3rscan}"
export COMET_EXPERIMENT_NAME="${COMET_EXPERIMENT_NAME:-Qwen3.5-0.8B | 3RScan psfr with_prev_gt clean | epochs=${EPOCHS}}"
...
exec "$SCRIPT_DIR/run_sft.sh" \
  ...
  --test "$SUBSET/test_clean.jsonl" \
  --exp_name 3rscan_psfr_with_prev_gt \
  --tuner_type full \
  --epochs "$EPOCHS" \
  "${NUM_EXTRA[@]}" \
  "$@"
