#!/usr/bin/env bash
# 3RScan base_annot (with_prev_gt): train on base_annot, validate on PSFR.
#
# Run (from this directory):
#   CUDA_VISIBLE_DEVICES=0 ./run_3rscan_base_annot.sh
#   CUDA_VISIBLE_DEVICES=0,1,2,3 ./run_3rscan_base_annot.sh
#   EPOCHS=10 ./run_3rscan_base_annot.sh
...
SUBSET="$R3SCAN/3RScan_json_base_annot_with_prev_gt"
VAL_SUBSET="$R3SCAN/3RScan_json_psfr_with_prev_gt"

EPOCHS="${EPOCHS:-5}"
export COMET_PROJECT_NAME="${COMET_PROJECT_NAME:-qwen_3_5_3rscan}"
export COMET_EXPERIMENT_NAME="${COMET_EXPERIMENT_NAME:-Qwen3.5-0.8B | 3RScan base_annot with_prev_gt clean | epochs=${EPOCHS}}"
...
exec "$SCRIPT_DIR/run_sft.sh" \
  ...
  --exp_name 3rscan_base_annot_with_prev_gt \
  --tuner_type full \
  --epochs "$EPOCHS" \
  "${NUM_EXTRA[@]}" \
  "$@"
