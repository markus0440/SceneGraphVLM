...
### 2.4 End-to-end pipeline (summary)
...
# 3) Optional: drop zero-relation rows (PSG uses rel[]{subj,pred,obj} — compatible with the cleaner)
python utils/annotations_clean/clean_zero_rel_frames.py datasets/data_playground/PSG_json
```

## 3. Optional: drop samples with zero relations

Same utility as for other datasets: `utils/annotations_clean/clean_zero_rel_frames.py`.

**PSG JSONL uses `rel[N]{subj,pred,obj}`**, so the detector in the script **matches** this format and can remove rows with zero declared / zero actual relation lines.
...
python utils/annotations_clean/clean_zero_rel_frames.py \
  datasets/data_playground/PSG_json \
  --report-name psg_zero_rel_cleanup_report.json
```

**Outputs**

- `datasets/data_playground/PSG_json/train_clean.jsonl`
- `datasets/data_playground/PSG_json/test_clean.jsonl`
- `datasets/data_playground/PSG_json/zero_rel_cleanup_report.json` (or custom `--report-name`)
...
│           ├── train.jsonl
│           ├── test.jsonl
│           ├── train_clean.jsonl       # optional clean step
│           ├── test_clean.jsonl
